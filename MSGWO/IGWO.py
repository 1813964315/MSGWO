...
#         print("The iteration is:", i + 1)
        ...
        # a = 2 * (1 - iratio)
        #对每个个体进行位置更新
        a = amax - amax*np.log(1+(np.e-1)*(i/imax))
        for j in range(m):
            ...
            X3 = Xdelta - A3 * Ddelta
            #计算个体移动后的位置及适应度值
            w = ((imax - i) / imax) * (w1 - w2) + w2  # 惯性权重
            pop[j] = w * ((X1 + X2 + X3) / 3) + 0.5 * np.random.rand() * (Xalpha - pop[j]) + 0.5 * np.random.rand() * (
                        X1 - pop[j])
            pop_fitness[j] = cec_fun(pop[j])
