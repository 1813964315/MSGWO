...
#         print("The iteration is:", i + 1)
        ...
        for j in range(m):
            ...
            X3 = Xdelta - A3 * Ddelta
            #计算个体移动后的位置及适应度值
            w1 = value(X1) / (value(X1) + value(X2) + value(X3) + d)
            w2 = value(X2) / (value(X1) + value(X2) + value(X3) + d)
            w3 = value(X3) / (value(X1) + value(X2) + value(X3) + d)
            #             print(value(X1)+value(X2)+value(X3)+d)
            pop[j] = ((w1 * X1 + w2 * X2 + w3 * X3) / 3) * (1 - i / imax) + X1 * (i / imax)
            pop_fitness[j] = cec_fun(pop[j])
