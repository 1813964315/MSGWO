...
#         print("The iteration is:", i + 1)
        ...
        # a = 2 * (1 - iratio)
        #对每个个体进行位置更新
        a = a1 - (a1-a2)*np.random.uniform()+np.random.normal()*0.1
        for j in range(m):
            ...
            X3 = Xdelta - A3 * Ddelta
            #计算个体移动后的位置及适应度值
            pop[j] = ((X1 + X2 + X3) / 3)+0.1* np.random.rand()*(Xalpha-pop[j])+0.9*np.random.rand()*(pop[np.random.randint(0,m-1)]-pop[j])
            pop_fitness[j] = cec_fun(pop[j])
