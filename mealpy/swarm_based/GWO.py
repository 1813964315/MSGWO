...
class OriginalEGWO(Optimizer):
    ...
    def evolve(self, epoch):
        ...
        mu = 0.001
        _, list_best, _ = self.get_special_solutions(self.pop, best=3)
        a = (2 * np.exp(-alpha * epoch**2) +
            beta * np.exp(-lambda_ * epoch) * np.cos(gamma * epoch* np.exp(-mu * epoch)) +
            delta * np.exp(-lambda_ * epoch) * np.sin(epsilon * epoch + phi) +
            zeta * np.log(1 + theta * epoch**2))
        pop_new = []
        for idx in range(0, self.pop_size):
            A1 = a * (2 * np.random.rand(self.problem.n_dims) - 1)
            A2 = a * (2 * np.random.rand(self.problem.n_dims) - 1)
...
class OriginalIEMGWO(Optimizer):
    """
    The original version of: Grey Wolf Optimizer (GWO)
